...
async function verifyStripeSetupIntent(input: {
  ...
  if (!response.ok) {
    throw new Error(json?.error?.message || "Could not verify saved card setup.")
  }
  ...
  if (json.payment_method !== input.paymentMethodId) {
    throw new Error("Saved card setup does not match the selected payment method.")
  }
}
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    const existingMetadata = metadataObject(
    ...
    await cartModule.updateCarts(cartId, { metadata: checkoutMetadata })
    ...
    await ensurePaymentSetup(db, {
      ...
      stripeCustomerId: accountHolder ? getStripeCustomerId(accountHolder) : null,
